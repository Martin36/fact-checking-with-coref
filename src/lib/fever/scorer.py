...
def evidence_macro_precision(instance, max_evidence=None):
    ...
    if instance["label"].upper() != "NOT ENOUGH INFO":
        all_evi = [[e[2], e[3]] for eg in instance["evidence"]
                   for e in eg if e[3] is not None]
    ...
    for idx, instance in enumerate(predictions):
        ...
        if 'evidence' not in instance or 'label' not in instance:
            ...
            assert len(actual) == len(
                predictions), 'actual data and predicted data length must match'
            assert 'evidence' in actual[idx].keys(
